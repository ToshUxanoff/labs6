...
struct NewSubj
{
	void operator() (Student& a)
	{
		...
		int Mark_Lit2 = (int)ceil(Mark_Lit);
		a.Grades.insert(std::pair<std::string, Grade>("Russian Literature",(Grade) Mark_Lit2));
	}
};
...
bool subj_compare(const Student &a, const Student &b)
{
	return (a.Grades.size() < b.Grades.size());
}
bool math_compare(const Student &a, const Student &b)
{
...
bool mark_compare(const Student &a, const Student &b)
{
	float Mark_A = 0;
	float Mark_B = 0;
